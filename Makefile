...
OASIS_SRCDIR ?= $(TOPDIR)
OASIS_DSTDIR ?= $(UPPERDIR)/awgrelease
...
github-build-artifacts: ## Run GitHub workflow to build OASIS OpenWrt packages (use WORKFLOW_REF to specify branch/tag)
	@{ \
	set -ex ; \
...
# Warning: ELOOP: too many symbolic links encountered, stat '/home/runner/work/OASIS-openwrt/OASIS-openwrt/openwrt/staging_dir/toolchain-mips_24kc_gcc-11.2.0_musl/initial/lib/lib'
...
build-oasis: ## Build OASIS-openwrt kernel module and packages
	...
	echo "src-cpy oasisopenwrt $(OASIS_SRCDIR)" >> feeds.conf ; \
...
prepare-artifacts: ## Save OASIS-openwrt artifacts from regular builds
	@{ \
	set -ex ; \
	cd $(OPENWRT_SRCDIR) ; \
	VERMAGIC=$$(cat ./build_dir/target-$(OPENWRT_ARCH)*/linux-$(OPENWRT_TARGET)_$(OPENWRT_SUBTARGET)/linux-*/.vermagic) ; \
	echo "Vermagic: $${VERMAGIC}" ; \
	mkdir -p $(OASIS_DSTDIR) ; \
	cp bin/packages/$(OPENWRT_ARCH)/oasisopenwrt/oasis-mod-spring_*.ipk $(OASIS_DSTDIR)/oasis-mod-spring_$(POSTFIX)_$${VERMAGIC}.ipk ; \
	cp bin/packages/$(OPENWRT_ARCH)/oasisopenwrt/oasis_*.ipk $(OASIS_DSTDIR)/oasis_$(POSTFIX)_$${VERMAGIC}.ipk ; \
	cp bin/packages/$(OPENWRT_ARCH)/oasisopenwrt/luci-app-oasis_*.ipk $(OASIS_DSTDIR)/luci-app-oasis_$(POSTFIX)_$${VERMAGIC}.ipk ; \
	cp bin/targets/$(OPENWRT_TARGET)/$(OPENWRT_SUBTARGET)/packages/oasis-mod-tool_*.ipk $(OASIS_DSTDIR)/oasis-mod-tool_$(POSTFIX)_$${VERMAGIC}.ipk ; \
	}
...
prepare-release: check-release ## Save OASIS-openwrt artifacts from tagged release
	@{ \
	set -ex ; \
	cd $(OPENWRT_SRCDIR) ; \
	mkdir -p $(OASIS_DSTDIR) ; \
	cp bin/packages/$(OPENWRT_ARCH)/oasisopenwrt/oasis-mod-spring_*.ipk $(OASIS_DSTDIR)/oasis-mod-spring_$(POSTFIX_RELEASE).ipk ; \
	cp bin/packages/$(OPENWRT_ARCH)/oasisopenwrt/luci-app-oasis_*.ipk $(OASIS_DSTDIR)/luci-app-oasis_$(POSTFIX_RELEASE).ipk ; \
	cp bin/targets/$(OPENWRT_TARGET)/$(OPENWRT_SUBTARGET)/packages/oasis-mod-tool_*.ipk $(OASIS_DSTDIR)/oasis-mod-tool_$(POSTFIX_RELEASE).ipk ; \
	cp bin/packages/$(OPENWRT_ARCH)/oasisopenwrt/oasis_*.ipk $(OASIS_DSTDIR)/oasis_$(POSTFIX_RELEASE).ipk
	}
